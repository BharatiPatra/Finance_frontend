...
const MfCard: React.FC<MfCardProps> = ({ transactions = [] }) => {
    ...
    <div className="w-full">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-white mb-2">
          Mutual Fund Transactions ({transactions.length})
        </h3>
        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs uppercase bg-gray-700 text-gray-300">
            <tr>
              ...
              <th scope="col" className="px-3 py-3">Scheme Name</th>
              <th scope="col" className="px-3 py-3">Type</th>
              <th scope="col" className="px-3 py-3">Units</th>
              <th scope="col" className="px-3 py-3">Price</th>
              <th scope="col" className="px-3 py-3">Amount</th>
              <th scope="col" className="px-3 py-3">Folio</th>
            </tr>
          </thead>
          <tbody>
      ...
      <div className="mt-6 p-4 bg-gray-700/50 rounded-lg">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className="text-white font-medium">{transactions.length}</p>
          </div>
          <div>
            <p className="text-gray-400">Total Investment</p>
            <p className="text-white font-medium">
              {formatCurrency({
                units: transactions.reduce((sum, t) => {
          ...
          <div>
            ...
            <p className="text-white font-medium">
              {transactions.reduce((sum, t) => sum + t.transactionUnits, 0).toLocaleString('en-IN', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 4
              })}
            </p>
          ...
          <div>
            ...
            <p className="text-white font-medium">
              {new Set(transactions.map(t => t.schemeName)).size}
            </p>
          </div>
        </div>
      </div>
